...
namespace UnityEngine.XR.MagicLeap
{
    ...
    public sealed class ControllerConnectionHandler : MonoBehaviour
    {
        ...
        public MLInputController ConnectedController
        {
            get
            {
                if (_allowedConnectedDevices.Count == 0)
                {
                    return null;
                }
                else {
                    for (int i = 0; i < _allowedConnectedDevices.Count; i++) {
                        if (_allowedConnectedDevices[i].Type == MLInputControllerType.Control) {
                            return _allowedConnectedDevices[i];
                        }
                    }
                    return null;
                }
        ...
        public MLInputController ConnectedMobileController
        {
            get
            {
                if (_allowedConnectedDevices.Count == 0)
                ...
                else
                {
                    for (int i = 0; i < _allowedConnectedDevices.Count; i++)
                    {
                        if (_allowedConnectedDevices[i].Type == MLInputControllerType.MobileApp)
                        {
                            return _allowedConnectedDevices[i];
                        }
                    }
